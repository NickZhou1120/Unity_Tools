...
public class UIAtlasMaker : EditorWindow
{
    ...
    static public void UpdateAtlas(UIAtlas atlas, List<SpriteEntry> sprites)
    {
        ...
        if (NGUISettings.atlas.texture != null)
            TextureSeprateAlpha.SetTextureToARGB(NGUISettings.atlas.texture);
        ...
        if (sprites.Count > 0)
        {
            ...
            #region 分离alpha
            if (TextureSeprateAlpha.WetherSeparate)
            {
                Texture rgb, a;
                TextureSeprateAlpha.SeparateTexture(NGUISettings.atlas.texture, out rgb, out a);
                NGUISettings.atlas.spriteMaterial.shader = Shader.Find("Alpha Separated Colored");
                NGUISettings.atlas.spriteMaterial.SetTexture("_MainTex", rgb);
                NGUISettings.atlas.spriteMaterial.SetTexture("_AlphaTex", a);
            }
            #endregion 
            return;
        }
    ...
    void OnGUI()
    {
        ...
        GUILayout.BeginHorizontal();
        TextureSeprateAlpha.WetherSeparate = EditorGUILayout.Toggle("Separate alpha", TextureSeprateAlpha.WetherSeparate, GUILayout.Width(100f));
        GUILayout.Label("or do nothing", GUILayout.MinWidth(70f));
        GUILayout.EndHorizontal();
